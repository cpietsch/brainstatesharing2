...
var TrackEditor = function  (trackPlayer) {
	...
	//     	.orient("right")
	// 	    .tickSize(width,0)
	// 	    .ticks(y.domain()[1]-y.domain()[0])
	// 	    .tickFormat("")
	// 	 )

	console.log((x.domain()[1]-x.domain()[0])*(y.domain()[1]-y.domain()[0]))


	// var gridData = d3.range(x.domain()[0],x.domain()[1]).map(function(x){
	// 	return d3.range(y.domain()[0],y.domain()[1]).map(function(y){
	// 		return {x:x,y:y};
	// 	})
	// })
